package com.example.demo.global.jwt.security;

import com.example.demo.domain.user.entity.UserRoleEnum;
import com.example.demo.domain.user.repository.UserRepository;
import com.example.demo.global.exception.CustomTokenException;
...
public class JwtUtil {
    ...
    // accessToken 만료시간
    private final long TOKEN_TIME = 3 * 24 * 60 * 60 * 1000L; // 30분
    ...
    public boolean validateToken(HttpServletRequest req, HttpServletResponse res, String token) {
        try {
            ...
            if (req.getHeader(REFRESH_TOKEN).isEmpty()) {
                ...
                logger.info("토큰재발급 성공: {}", newAccessToken);
            }
    ...
    private String regenerateAccessToken(String refreshToken) {
        ...
        if (RT == null) {
            ...
            Optional<com.example.demo.domain.user.entity.User> userOptional = userRepository.findByEmail(RT);
...
//        return null;
    ...
    public String generateToken(UserDetailsImpl userDetails) {
        return createRefreshToken(userDetails.getUser().getUserId());
    }
}
